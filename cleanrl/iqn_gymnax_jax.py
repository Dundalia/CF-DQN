"""
Implicit Quantile Networks (IQN, Dabney et al. 2018) — Equinox + vectorised gymnax/Craftax.
"""
import os
import time
...
def quantile_huber_loss_iqn(u: jnp.ndarray, tau_i: jnp.ndarray, kappa: float) -> jnp.ndarray:
    """u: (B, N_tau, N_tau_p); tau_i: (N_tau,) sampled τ for current quantiles."""
...
class Args:
    ...
    hidden3: int = 0
    n_cos: int = 64
    """number of cosine basis terms π i τ"""
    quantile_embedding_dim: int = 64
    num_tau_samples: int = 32
    """K_τ for training (online)"""
    num_tau_prime_samples: int = 32
    """K_τ' for target"""
    num_quantile_samples: int = 32
    """K for inference Q = mean over samples"""
    kappa: float = 1.0


class IQNNetwork(eqx.Module):
    """State trunk + τ embedding; concat(h, φ(τ)) → per-action quantile values."""
    ...
    def __init__(
        ...
        *,
        ...
        self.embed_w = eqx.nn.Linear(n_cos, quantile_embedding_dim, key=keys[3])
        self.head = eqx.nn.Linear(h_out + quantile_embedding_dim, action_dim, key=keys[4])
    ...
    def __call__(self, obs: jnp.ndarray, tau: jnp.ndarray) -> jnp.ndarray:
        """obs (obs_dim,), tau (K,) → (K, action_dim) quantile values."""
        h = self._trunk(obs)
        emb = self._tau_embed(tau)
        k = tau.shape[0]
    ...
    def q_values(self, obs: jnp.ndarray, key: jnp.ndarray, num_samples: int) -> jnp.ndarray:
        ...
        tau = jax.random.uniform(k2, (num_samples,))
        z = self(obs, tau)
...
def make_train(args: Args):
    ...
    def gradient_step(q_net, target_net, opt_state, rb, key):
        ...
        def loss_fn(net):
            online_q_next = q_mean_online(net, s_next_obs, tau_online)
            next_actions = jnp.argmax(online_q_next, axis=1)
            theta = z_sa(net, s_obs, s_actions, tau_online)
            theta_t = z_sa(target_net, s_next_obs, next_actions, tau_prime)
            Y = s_rewards[:, None] + args.gamma * (1.0 - s_dones[:, None]) * theta_t
            Y = jax.lax.stop_gradient(Y)
            u = Y[:, None, :] - theta[:, :, None]
            loss = quantile_huber_loss_iqn(u, tau_online, args.kappa)
            z_all = jax.vmap(lambda o: net(o, tau_online))(s_obs)
            z_m = jnp.mean(z_all, axis=1)
            q_mean = z_m[batch_idx, s_actions].mean()
            q_gap = (z_m.max(axis=1) - z_m.min(axis=1)).mean()
    ...
    def train_step(runner_state, unused):
        ...
        q_values = jax.vmap(lambda o, k: q_net.q_values(o, k, args.num_quantile_samples))(obs, infer_keys)
